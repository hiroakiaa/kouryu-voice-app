...
export function groupHistory(items=[]){const sorted=[...items].sort((a,b)=>(b.at||0)-(a.at||0)).slice(0,40),groups=[];for(const item of sorted){const prev=groups.at(-1),day=new Date(item.at||0).toDateString();if(prev&&prev.number===item.number&&prev.direction===item.direction&&prev.day===day){prev.count++;prev.entries.push(item);continue;}groups.push({...item,count:1,day,entries:[item]});}return groups.slice(0,20);}
