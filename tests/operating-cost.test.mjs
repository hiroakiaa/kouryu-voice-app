...
test('全機能の試算は人数・為替・共有たとえの再利用を反映する',()=>{
 ...
 assert.ok(Math.abs(two.low-11.90598)<1e-6);assert.ok(Math.abs(four.high-24.04866)<1e-6);
...
test('人数と利用条件を変えると料金画面・ヒントを同じ試算に更新する',()=>{
 ...
 assert.equal(nodes.get('#costScenarioTotal').textContent,'11.91円〜12.05円');assert.equal(nodes.has('#appInfoCostSummary'),false);
 nodes.get('#costParticipants').value='4';nodes.get('#costParticipants').change();
 assert.equal(nodes.get('#tipsTenMinuteCost').textContent,'23.18円〜24.05円');
